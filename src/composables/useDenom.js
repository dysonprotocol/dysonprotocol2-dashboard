...
export function useDenom() {
  // REST base is always current origin
  ...
  async function loadDenomMetadata() {
    ...
    if (Array.isArray(denomMetadatas.value) && denomMetadatas.value.length > 0) return
    ...
    _denomState.loadPromise = (async () => {
      try {
        const resp = await api.get(`/cosmos/bank/v1beta1/denoms_metadata`, {
  ...
  async function ensureDenomsLoaded() {
    const isEmpty = !Array.isArray(denomMetadatas.value) || denomMetadatas.value.length === 0
    if (!isEmpty) return
  ...
  function normalizeFromDisplay({ amount, displayDenom }) {
    ...
    if (!metadata || !displayUnit) throw new Error(`Denom metadata not found for ${denom}`)
  ...
  function normalizeCoin({ amount, denom }) {
    ...
    if (!displayUnit) throw new Error(`Display unit not found for ${displayDenom}`)
