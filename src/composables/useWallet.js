...
export function useWallet() {
  ...
  const handleKeplrAccountChange = async () => {
    ...
    try {
      ...
      const newKeplrWallet = { name: key.name, address: key.bech32Address, type: 'keplr' }
  ...
  const unlockWallet = async (name, password) => {
    ...
    if (existingIndex === -1) {
      unlockedWallets.value.push({ name, address, type: COSMJS_WALLET_TYPE, _pass: password })
    } else {
  ...
  const importNamedCosmJsWallet = async (name, mnemonic, password) => {
    ...
    localCosmJsWallets.value.push({ name: name.trim(), encrypted, _pass: password, address })
  }
  ...
  const getWallet = async (overrideAddress) => {
    if (!overrideAddress) throw new Error('Explicit address required for getWallet()')
  ...
  const sendMsg = async ({
    ...
    if (!executorAddress) throw new Error('executorAddress is required in sendMsg()')
  ...
  const runDysonScript = async ({
    ...
    if (gasLimit === 'auto' && !simulate) {
      if (type === COSMJS_WALLET_TYPE) {
        ...
        if (!simulationResult.success) return simulationResult
        let gasUsed = 0
        if (simulationResult?.raw?.gas_info?.gas_used)
          gasUsed = parseInt(simulationResult.raw.gas_info.gas_used)
        else if (simulationResult?.gasUsed) gasUsed = parseInt(simulationResult.gasUsed)
    ...
    const result = { kind, success, scriptResponse, rawSendMsgsResponse: sendResult }
    if (!simulate) {
  ...
  const signArbitraryData = async ({ address, data = '', msg = null }) => {
    ...
    transaction.auth_info.signer_infos = [
      {
        public_key: { '@type': '/cosmos.crypto.secp256k1.PubKey', key: toBase64(pubkey) },
    ...
    if (!encodeRes.ok) throw new Error(`Failed to encode: ${await encodeRes.text()}`)
  ...
  const addTransaction = (txMetadata) => {
    ...
    if (txHistory.value.length > 100) txHistory.value = txHistory.value.slice(0, 100)
  }
