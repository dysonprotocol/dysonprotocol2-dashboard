...
import { getChainInfo, sendMsgs, runScript } from './dysonTxUtils.js'
import { TxRaw } from 'cosmjs-types/cosmos/tx/v1beta1/tx.js'
import { toBase64, fromBase64 } from '@cosmjs/encoding'

const DEFAULT_CHAIN_INFO = null
...
document.addEventListener('alpine:init', () => {
  Alpine.store('walletStore', {
    ...
    getDisplayTextForOption(option) {
    ...
    async signArbitraryData({ data }) {
      const { walletInstance, address } = this.getWallet()
    ...
    async loadDenomMetadata() {
      ...
      const resp = await fetch(url)
      if (!resp.ok) {
        console.warn(`Failed to load denom metadata: ${resp.status} ${resp.statusText}`)
        return
      }
      const json = await resp.json()
