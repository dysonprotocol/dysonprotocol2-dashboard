<template>
  <h2 class="text-xl font-semibold" id="account">Account</h2>
  <section class="grid gap-6 md:grid-cols-3">
    <div class="space-y-2 p-4 border rounded">
      <h3 class="font-semibold">Account</h3>
      <div class="grid gap-2 md:grid-cols-3">
        <input v-model="address" class="input w-full" placeholder="address" />
        <button class="btn btn-primary" @click="loadAccountInfo">Account Info</button>
        <button class="btn btn-primary" @click="loadAccountDetails">Account</button>
      </div>
      <div class="text-sm">
        <div v-if="account">
          account_number: <code>{{ account.account_number }}</code
          >, sequence:
          <code>{{ account.sequence }}</code>
        </div>
        <div v-else class="opacity-70">No account loaded</div>
      </div>
    </div>

    <div class="space-y-2 p-4 border rounded">
      <h3 class="font-semibold">Auth Params</h3>
      <button class="btn btn-primary" @click="loadAuthParams">Load Params</button>
      <div class="text-sm">
        <div>
