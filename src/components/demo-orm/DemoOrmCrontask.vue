<template>
  <h2 id="crontask" class="text-xl font-semibold">Crontask</h2>
  <section class="grid gap-6 md:grid-cols-3">
    <div class="space-y-4 p-4 border rounded">
      ...
      <div class="space-y-2">
        ...
        <div class="space-y-2">
          ...
          <div v-if="byCreatorError" class="text-sm text-red-600">
            {{ byCreatorError }}
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div v-if="statusTsError" class="text-sm text-red-600">
            {{ statusTsError }}
          </div>
          <div v-if="statusGasError" class="text-sm text-red-600">
            {{ statusGasError }}
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div v-if="byIdError" class="text-sm text-red-600">
            {{ byIdError }}
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div v-if="allError" class="text-sm text-red-600">
            {{ allError }}
          </div>
        </div>
      </div>
    ...
    <div class="space-y-2 p-4 border rounded">
      ...
      <div v-if="taskError" class="text-sm text-red-600">
        {{ taskError }}
      </div>
    </div>
...
async function fetchByCreator() {
  ...
  try {
    ...
    await useAxiosRepo(CrontaskTask)
      .api()
      .fetchByCreatorInit({ creator: creator.value, reverse: true })
  } catch (e: any) {
    console.error(e)
