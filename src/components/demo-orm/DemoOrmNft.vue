<template>
  <h2 class="text-xl font-semibold" id="nft">NFT</h2>
  <section class="grid gap-6 md:grid-cols-3">
    <div class="space-y-4 p-4 border rounded">
      <h3 class="font-semibold">Queries</h3>
      <div class="space-y-2">
        <div class="space-y-2">
          <h4 class="text-sm font-semibold opacity-70">Balance</h4>
          <input v-model="balClassId" class="input w-full" placeholder="class_id" />
          <input v-model="balOwner" class="input w-full" placeholder="owner" />
          <button class="btn btn-primary" @click="fetchBalance">Fetch</button>
          <div class="text-sm">
            amount=<code>{{ balanceAmount }}</code>
          </div>
          <div v-if="balError" class="text-sm text-red-600">{{ balError }}</div>
        </div>

        <div class="space-y-2">
          <h4 class="text-sm font-semibold opacity-70">NFT by ID</h4>
          <input v-model="nftClassId" class="input w-full" placeholder="class_id" />
          <input v-model="nftId" class="input w-full" placeholder="id" />
          <div class="flex gap-2">
            <button class="btn btn-primary" @click="fetchNft">Fetch</button>
            <button class="btn btn-primary" @click="fetchOwner">Fetch Owner</button>
          </div>
          <div class="text-sm">
            owner=<code>{{ nftOwner }}</code>
          </div>
          <div v-if="nftError" class="text-sm text-red-600">{{ nftError }}</div>
        </div>

        <div class="space-y-2">
          <h4 class="text-sm font-semibold opacity-70">Supply</h4>
          <input v-model="supplyClassId" class="input w-full" placeholder="class_id" />
          <button class="btn btn-primary" @click="fetchSupply">Fetch</button>
          <div class="text-sm">
            amount=<code>{{ supplyAmount }}</code>
          </div>
          <div v-if="supplyError" class="text-sm text-red-600">{{ supplyError }}</div>
        </div>

        <div class="space-y-2">
          <h4 class="text-sm font-semibold opacity-70">NFTs (by class_id and/or owner)</h4>
          <input v-model="listClassId" class="input w-full" placeholder="class_id (optional)" />
          <input v-model="listOwner" class="input w-full" placeholder="owner (optional)" />
          <div class="flex gap-2">
            <button class="btn btn-primary" @click="fetchNftsInit">Init</button>
          </div>
          <div class="text-sm">
            count=<code>{{ nftsCount }}</code>
          </div>
          <div v-if="nftsError" class="text-sm text-red-600">{{ nftsError }}</div>
        </div>

        <div class="space-y-2">
          <h4 class="text-sm font-semibold opacity-70">Classes</h4>
          <div class="flex gap-2">
            <input v-model="classId" class="input w-full" placeholder="class_id" />
            <button class="btn btn-primary" @click="fetchClass">Fetch</button>
          </div>
          <div class="flex gap-2">
            <button class="btn btn-primary" @click="fetchClasses">Init</button>
          </div>
          <div class="text-sm">
    ...
    <div class="space-y-2 p-4 border rounded">
      <h3 class="font-semibold">NFTs (in memory)</h3>
      ...
      <div class="overflow-x-auto">
        <table class="table table-xs w-full">
          ...
          <tbody>
            <tr v-for="r in nftList" :key="r.key">
              <td class="font-mono">{{ r.class_id }}</td>
      ...
      <h4 class="text-sm font-semibold opacity-70">Balances (in memory)</h4>
      <div class="text-sm opacity-70">
      ...
      <div class="overflow-x-auto">
        <table class="table table-xs w-full">
          ...
          <tbody>
            <tr v-for="b in balancesList" :key="b.class_id + ':' + b.owner">
              <td class="font-mono">{{ b.class_id }}</td>
              <td>
                <code>{{ b.owner }}</code>
              </td>
              <td>
      ...
      <div class="h-px bg-gray-200" />
      <h4 class="text-sm font-semibold opacity-70">Classes (in memory)</h4>
      <div class="text-sm opacity-70">
      ...
      <div class="overflow-x-auto">
        <table class="table table-xs w-full">
          ...
          <tbody>
            <tr v-for="c in classesList" :key="c.id">
      ...
      <div class="h-px bg-gray-200" />
      <h4 class="text-sm font-semibold opacity-70">Supply (in memory)</h4>
      <div class="text-sm opacity-70">
      ...
      <div class="overflow-x-auto">
        <table class="table table-xs w-full">
          ...
          <tbody>
            <tr v-for="s in suppliesList" :key="s.class_id">
              <td class="font-mono">{{ s.class_id }}</td>
              <td>
                <code>{{ s.amount }}</code>
              </td>
            </tr>
    ...
    <div class="space-y-2 p-4 border rounded">
      <h3 class="font-semibold">Send NFT</h3>
      <input v-model="sendClassId" class="input w-full" placeholder="class_id" />
      <input v-model="sendId" class="input w-full" placeholder="id" />
      <input v-model="sendFrom" class="input w-full" placeholder="sender" />
      <input v-model="sendTo" class="input w-full" placeholder="receiver" />
      <input v-model="sendMemo" class="input w-full" placeholder="memo (optional)" />
      <button class="btn btn-primary" @click="sendNft">Send</button>
