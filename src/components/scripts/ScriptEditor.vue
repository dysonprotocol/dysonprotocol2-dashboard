    ...
    <div class="flex justify-between items-center p-4">
      <div class="flex gap-2 items-center">
        ...
        <div v-if="errorMessage" class="text-error text-sm ml-2">
          {{ errorMessage }}
          <button @click="clearError" class="btn btn-xs btn-ghost ml-1">
            ✕
          </button>
        </div>
...
const selectedEditorExecutor = useStorage(
  () => `executor:edit:${props.address}`,
  props.address
);
...
const source = computed(() =>
  props.script ? props.script.code ?? "" : defaultCode
);

const hasChanges = computed(
  () => canEdit.value && currentContent.value !== originalContent.value
);
...
async function save() {
  ...
  try {
    ...
    const hasWallet = wallet.unlockedWallets.value?.some(
      (w) => w.address === props.address
    );
    if (!hasWallet) {
      throw new ScriptStoreError(
        "Unlock the wallet that controls this script address to save"
      );
    }
...
function highlightRange(line, startColRaw, endLine, endColRaw) {
  ...
  const endCol = Math.max(
    1,
    Number(endColRaw ?? Number(startColRaw ?? 0) + 1) + 1
  );
