    ...
    <div class="collapse-content">
      <div class="form-control min-w-0">
        ...
        <div class="text-xs opacity-60 wrap-anywhere">
          This code will be temporarily appended to the script before calling
          the function
        </div>
...
const editorTheme = computed(() =>
  globalThis.__dyson_theme === "dark" ? "vs-dark" : "vs"
);
...
const isAddressKnown = computed(() => {
  const unlocked = unlockedWallets.value?.some(
    (w) => w.address === props.address
  );
  if (unlocked) return true;
  const locked = localCosmJsWallets.value?.some(
    (w) => w.address === props.address
  );
  return !!locked;
});
...
watch(scriptLineCount, () => {
  if (editor)
    editor.updateOptions({ lineNumbers: (ln) => ln + scriptLineCount.value });
});
