            ...
            <Alert v-if="errorText" variant="destructive" class="mt-3 break-all">
              ...
              <AlertDescription>
                ...
                <div v-if="exception" class="mt-2 text-xs">
                  <Button variant="link" class="h-auto p-0" @click="goToException"
                    >Go to line {{ exception.lineno }}:{{ exception.col_offset }}</Button
                  >
                </div>
              </AlertDescription>
            </Alert>
...
const extraCodeStorage = useStorage<Record<string, string>>('script-extra-codes', {})
const extraCode = ref('')

const isCollapsedStore = useStorage<boolean>('script-extra-code-collapsed', false)
...
const isUnlocked = computed(
  () => unlockedWallets.value?.some((w: any) => w.address === props.address) || false
)
...
function highlightError(ex: any) {
  ...
  const startLine = Math.max(1, Number(ex.lineno || 1) - Number(scriptLineCount.value || 0))
  const endLine = Math.max(
    1,
    Number(ex.end_lineno || Number(ex.lineno || 1)) - Number(scriptLineCount.value || 0)
  )
...
async function refreshSpendables(addr: string) {
  ...
  const list = ((repo.all() as any[]) || []).filter((s: any) => String(s.address) === addr)
