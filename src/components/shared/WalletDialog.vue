      ...
      <div class="space-y-6">
        ...
        <section class="space-y-2">
          <div v-if="hasKeplrWallet" class="border rounded-md p-4">
            ...
            <div class="mt-2 font-mono text-xs break-all">{{ keplrWallet?.address }}</div>
          </div>
          <div v-else class="space-y-2">
            ...
            <div v-if="keplrError" class="text-destructive text-xs" data-testid="keplr-error">
              {{ keplrError }}
            </div>
          </div>
        </section>
        ...
        <section class="space-y-2">
          ...
          >
            <div class="p-3">
              <div class="flex items-center justify-between">
                <div class="font-medium" :data-testid="`wallet-name-${wallet.name}`">
                  {{ wallet.name }}
                </div>
                <div class="flex items-center gap-2">
                  <Button
              ...
              <div class="mt-3 flex items-center gap-2">
                <div v-if="!isWalletUnlocked(wallet)" class="flex items-center gap-2">
                  <Input
                    type="password"
                    ...
                    @keyup.enter="(e:any) => !unlockLoading[wallet.name] && handleInlineUnlock(wallet, e.target.value)"
                  />
                  <Button
                    size="xs"
                    :disabled="unlockLoading[wallet.name]"
            ...
            <div class="space-y-3">
              ...
              <label class="flex items-start gap-2 text-xs">
                <input
                  ...
                  required
                />
                <span class="text-muted-foreground">I've backed up my recovery phrase</span>
              </label>
...
function handleUnlockClick(wallet) {
  ...
  if (input && input.value && input.value.trim()) handleInlineUnlock(wallet, input.value)
}
...
function handleRemoveWallet(walletName) {
  const confirmed = confirm(`Remove wallet "${walletName}"? This cannot be undone.`)
