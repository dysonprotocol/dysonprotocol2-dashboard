<template>
  <div class="space-y-4">
      ...
      <div class="card-body space-y-2">
        ...
        <div :class="enabledClass(1)" class="space-y-2">
          <div class="join w-full">
            ...
            />
            <div id="name-suffix" class="input join-item w-16">.dys</div>
          </div>
          <div class="text-sm">
            ...
            <span v-else-if="nameMain && isValidName && nameAvailable" class="text-success"
              >The name {{ chosenName() }} is available</span
            >
            <span v-else-if="nameMain && isValidName && !nameAvailable" class="text-error"
    ...
    >
      ...
      <div class="card card-border" :class="cardClass(2)">
        <div class="card-body space-y-2">
          ...
          <p class="text-sm">Set the valuation and denom. Annual fee is charged at reveal.</p>
          <div :class="enabledClass(2)" class="space-y-2">
            <div class="join w-full">
              ...
              >
                ...
                <option v-for="opt in allowedDisplayOptions" :key="opt.base" :value="opt.display">
                  {{ opt.display }}
                </option>
              </select>
            </div>
            ...
            <div class="text-error text-xs" v-if="validationMessage">{{ validationMessage }}</div>
      ...
      <div class="card card-border bg-base-100" :class="cardClass(3)">
        <div class="card-body space-y-2">
          <div class="flex items-center justify-between">
          ...
          <p class="text-sm">Choose a wallet to sign commit and reveal.</p>
          <div :class="enabledClass(3)" class="space-y-2">
            ...
            />
            <div class="text-xs opacity-70" v-if="selectedExecutor">
              Selected: {{ selectedExecutor }}
            </div>
            <div class="text-error text-sm" v-else>No wallet selected.</div>
          </div>
        </div>
      </div>

      <!-- Step 4: Submit commitment -->
      <div class="card card-border bg-base-100" :class="cardClass(4)">
        <div class="card-body space-y-2">
          <div class="flex items-center justify-between">
          ...
          <div :class="enabledClass(4)" class="space-y-2">
            <div class="overflow-x-auto">
              <table class="table table-zebra">
                <tbody>
                  ...
                  <tr>
                    <td class="font-semibold">Annual fee</td>
            ...
            <div class="card-actions justify-end">
              <button
                ...
                :disabled="isBusy || !canCommit || commitTxHash"
                @click="commit"
            ...
            <div class="text-error text-xs" v-if="error && !commitTxHash">{{ error }}</div>
          </div>
        </div>
      </div>

      <!-- Step 5: Reveal -->
      <div class="card card-border bg-base-100" :class="cardClass(5)">
        <div class="card-body space-y-2">
          <div class="flex items-center justify-between">
          ...
          <div :class="enabledClass(5)" class="space-y-2">
            ...
            <div class="text-error text-xs" v-if="error && !revealTxHash">{{ error }}</div>
          </div>
        </div>
      </div>

      <!-- Step 6: Done -->
      <div class="card card-border bg-base-100" :class="cardClass(6)">
        <div class="card-body space-y-2">
          <h2 class="card-title">6. Congratulations!</h2>
          <p class="text-sm">Your name is now registered. You can visit its page to manage it.</p>
          <div class="card-actions justify-end">
            <router-link
              :to="`/names/${chosenName()}`"
    ...
    <div class="text-error" v-if="globalError">{{ globalError }}</div>
  </div>
</template>
...
const canCommit = computed(
  () => isValidName.value && hasNumericValuation.value && isAnyWalletConnected.value
)
const canReveal = computed(
  () =>
    Boolean(hexHash.value) && Boolean(salt.value) && isValidName.value && isAnyWalletConnected.value
)
