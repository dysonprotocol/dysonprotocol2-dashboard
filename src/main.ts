...
if (import.meta.env.DEV && typeof window !== 'undefined') {
  ...
  type OrmGlobal = Record<string, { model: typeof OrmModel; repo: RepoLike; api: () => unknown }>

  try {
    const modules = import.meta.glob('./orm/models/**/*.ts', { eager: true }) as Record<
      string,
    ...
    console.log('[dev] __orm exposed on window with models:', Object.keys(exposed))
  } catch (e) {
  ...
  try {
    ...
    const sendMsg = async (args: unknown) => {
      await ensureInit()
      return (wallet as unknown as { sendMsg: (a: unknown) => Promise<unknown> }).sendMsg(args)
    }
    ;(
