        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <form class="space-y-4" @submit.prevent="handleSearch">
            ...
            <div class="bg-base-200 rounded-box p-4">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="form-control">
                  ...
                  <select v-model="searchForm.limit" class="select select-bordered">
            ...
            <div class="card-actions">
              ...
              <button v-if="hasResults" type="button" class="btn btn-ghost" @click="clearResults">
                Clear Results
              </button>
            </div>
          </form>
          ...
          <div>
            ...
            <div v-else class="space-y-4">
              ...
              >
                <div class="text-sm font-medium mb-2">{{ w.name }} — {{ w.address }}</div>
                <ul class="space-y-2 text-sm">
                  <li>
                    <button class="link" @click="applyQuickQuery(`message.sender='${w.address}'`)">
                      All Transactions
                    </button>
                  </li>
                  <li>
                    <button
                      class="link"
                      @click="applyQuickQuery(`coin_spent.spender='${w.address}'`)"
                    >
                      Coins sent from
                    </button>
                  </li>
                  <li>
                    <button
                      class="link"
                      @click="applyQuickQuery(`coin_received.receiver='${w.address}'`)"
                    >
                      Coins sent to
                    </button>
                  </li>
    ...
    <div v-else-if="searchResults" class="space-y-4">
      ...
      <div v-if="limitedFromHeight" class="alert alert-warning">
        <span>
          ...
          >. Note, this is managed by the node's <code>min-retain-blocks</code> setting and is
          different from the <code>pruning</code> settings.
        </span>
      </div>
      <!-- Results Header -->
      ...
      <div v-if="searchResults.tx_responses?.length > 0" class="space-y-4">
        <div v-for="(tx, index) in searchResults.tx_responses" :key="tx.txhash || index">
      ...
      <!-- Pagination -->
      <div v-if="searchResults.tx_responses?.length > 0" class="flex justify-center mt-8">
        <div class="join">
          <button
            class="join-item btn"
