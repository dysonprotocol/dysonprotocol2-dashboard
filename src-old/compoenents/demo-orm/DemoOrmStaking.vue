<template>
  <h2 class="text-xl font-semibold" id="staking">Staking</h2>
  <section class="grid gap-6 md:grid-cols-3">
    <div class="space-y-2 p-4 border rounded">
      <h3 class="font-semibold">Delegations</h3>
      <div class="grid gap-2 md:grid-cols-2">
        <input v-model="delegatorAddr" class="input w-full" placeholder="delegator address" />
        <button class="btn btn-primary" @click="loadDelegations">Load Delegations</button>
      </div>
      <div v-if="delegationsError" class="text-sm text-red-600">{{ delegationsError }}</div>
      <ul class="list-disc pl-6">
        <li v-for="d in delegations" :key="d.delegator_address + ':' + d.validator_address">
    ...
    <div class="space-y-2 p-4 border rounded">
      <h3 class="font-semibold">Validators (first 20)</h3>
      <div class="flex gap-2">
        <button class="btn btn-primary" @click="loadValidators">Load Validators</button>
      </div>
      <div v-if="validatorsError" class="text-sm text-red-600">{{ validatorsError }}</div>
      <ul class="list-disc pl-6">
        <li v-for="v in validators.slice(0, 20)" :key="v.operator_address">
    ...
    <form class="space-y-2 p-4 border rounded" @submit.prevent="submitDelegate">
      <fieldset class="space-y-2">
        ...
        <div class="flex gap-2">
          <input v-model="delAmount" class="input w-full" placeholder="amount" />
          <input v-model="delDenom" class="input w-28" placeholder="denom" />
        </div>
        <input v-model="delMemo" class="input w-full" placeholder="memo (optional)" />
        <button class="btn btn-primary" type="submit">Delegate</button>
        <div v-if="delegateError" class="text-sm text-red-600">{{ delegateError }}</div>
      </fieldset>
    </form>

    <form class="space-y-2 p-4 border rounded" @submit.prevent="submitRedelegate">
      <fieldset class="space-y-2">
        <legend class="text-sm font-semibold opacity-70">
          /cosmos.staking.v1beta1.MsgBeginRedelegate
        </legend>
        <input v-model="redFrom" class="input w-full" placeholder="delegator address" />
        <input v-model="srcValoper" class="input w-full" placeholder="src valoper" />
        <input v-model="dstValoper" class="input w-full" placeholder="dst valoper" />
        <div class="flex gap-2">
          <input v-model="redAmount" class="input w-full" placeholder="amount" />
          <input v-model="redDenom" class="input w-28" placeholder="denom" />
        </div>
        <input v-model="redMemo" class="input w-full" placeholder="memo (optional)" />
        <button class="btn btn-primary" type="submit">Redelegate</button>
        <div v-if="redelegateError" class="text-sm text-red-600">{{ redelegateError }}</div>
      </fieldset>
    </form>
  </section>
</template>
