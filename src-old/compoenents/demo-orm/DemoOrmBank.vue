<template>
  ...
  <section class="grid gap-6 md:grid-cols-3">
    ...
    <div class="space-y-2 p-4 border rounded">
      ...
      >
      <input
        v-model="balanceDenom"
        class="input w-full"
        placeholder="denom"
      >
      <button
        class="btn btn-primary"
        @click="loadBalanceByDenom"
      >
        Load
      </button>
      <div class="text-sm opacity-70">
        Balances for {{ balanceAddr || '—' }} shown above
      </div>
      <ul class="list-disc pl-6">
        <li
          v-for="b in balances"
          :key="b.address + ':' + b.denom"
        >
    ...
    <div class="space-y-2 p-4 border rounded">
      ...
      <div class="flex gap-2">
        <button
          class="btn btn-primary"
          @click="loadSpendableAll"
        >
          All
        </button>
        <input
          v-model="spendableAddr"
          class="input w-full"
          placeholder="address"
        >
        <input
          v-model="spendableDenom"
          class="input w-full"
          placeholder="denom"
        >
        <button
          class="btn btn-primary"
          @click="loadSpendableByDenom"
        >
          By Denom
        </button>
      </div>
      <ul class="list-disc pl-6 text-sm">
        <li
          v-for="s in spendables"
          :key="s.address + ':' + s.denom"
        >
    ...
    <div class="space-y-2 p-4 border rounded">
      ...
      <div class="flex gap-2">
        <button
          class="btn btn-primary"
          @click="loadMetadataAll"
        >
          All
        </button>
        <input
          v-model="metadataDenom"
          class="input w-full"
          placeholder="denom"
        >
        <button
          class="btn btn-primary"
          @click="loadMetadataOne"
        >
          One
        </button>
      </div>
    ...
    <div class="space-y-2 p-4 border rounded">
      ...
      <button
        class="btn btn-primary"
        @click="loadSendEnabled"
      >
        Load
      </button>
      <ul class="list-disc pl-6 text-sm">
        <li
          v-for="e in sendEnabledList"
          :key="e.denom"
        >
    ...
    <div class="space-y-2 p-4 border rounded">
      ...
      <div class="grid gap-2 md:grid-cols-4">
        ...
        <button
          class="btn btn-primary"
          @click="loadDenomOwners"
        >
          By Path
        </button>
        <button
          class="btn btn-primary"
          @click="loadDenomOwnersByQuery"
        >
          By Query
        </button>
      </div>
      <ul class="list-disc pl-6 text-sm max-h-56 overflow-auto">
        <li
          v-for="o in denomOwners"
          :key="o.denom + ':' + o.address"
        >
